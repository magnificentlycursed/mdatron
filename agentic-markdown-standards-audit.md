...
## Positioning (operator ruling 2026-09-19)

The Claude Code + GitHub Copilot use cases are the LEAD adoption pitch: the silent failure modes above (subagent skip conditions, six-field strict upload, CLAUDE.md-shadows-AGENTS.md, applyTo-less instructions) are current, felt, and warned about by NO tool — mdatron-in-CI is the only pre-flight for the agent-config file class. Governing .claude/ + .github/ is the adoption wedge (narrow one-step CI add, incremental expansion to project docs); cross-assistant instruction drift (CLAUDE.md vs copilot-instructions.md vs AGENTS.md) is a pin-family problem no other tool addresses. Refinement (operator, same date): 'found out days later' is one of the BETTER outcomes — the modal case is NEVER finding out: agent output is stochastic, so a silently-skipped config shifts the distribution without a legible failure; no feedback loop, unfalsifiable from inside the mechanism (the disconnected smoke detector — false confidence compounds). Lead pitch language = false-confidence/never-find-out, not debugging-cost; the only detection is an external check OF the mechanism, which is mdatron's category — 'silence always means verified-clean' pointed outward (the same doctrine behind the envelope's families tri-state). Cookbook seeded-violation sections are FALSIFICATION DEMOS: reader proves the silent failure against the real tool first, then sees mdatron refuse it in CI. Second refinement (operator, same date): showcase the FAILURE LOOP, not just the golden path — detection AND course correction. Each recipe's failure-loop section shows: the rustc-style TTY diagnostic (rustc --explain pedigree; every code resolves in explain, CI-enforced); the SAME finding's structured envelope view for the agent (quoted[] regions marked untrusted + escape-hardened — the diagnostic itself cannot prompt-inject even when the broken config carries instruction-shaped text; no competing tool can claim this); mdatron explain <code> as remediation; the fix + clean re-run with the fingerprint identity proving the finding gone across runs. Frame as agent self-correction where honest (agent breaks config -> envelope lands in agent context -> agent fixes its own file -> green). README quickstart (#180) likewise shows one full loop, not success-only output. Recipe priority: SKILL.md (both profiles) -> Claude Code subagents -> Copilot .instructions.md -> llms.txt -> the tier-3 project-extension lesson. Tracking: crosslink #181.
