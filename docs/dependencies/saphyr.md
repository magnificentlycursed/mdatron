...
2026-07). The happy path uses `serde_yaml_ng`; saphyr runs only when a
...
- `serde-saphyr` (span-capable serde front end): the eventual convergence
  target (alpha); tracked for a future cycle. saphyr is the lower-level
...
- **Threat model:** parses adopter YAML on the error path. A panic in the
  ...
  run (verified at adoption, 2026-07).
