...
- **Pattern ReDoS (the driver for this upgrade):** adopter `pattern`s now run on the linear-time `regex` engine (`PatternOptions::regex()`), not backtracking `fancy-regex` — no catastrophic backtracking on contributor-supplied frontmatter
...
Single-author investigation; no ceremonial co-author trailers — this entry is the actual investigation.
