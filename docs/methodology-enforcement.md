...
| governed-estate register | the naming register holds over the living governance documents — `DESIGN.md`, the README, and `docs/` (`vocabulary_globs`, widened by #204), with the bold-means-coinage convention scoped to the methodology docs (`coinage_globs`) | enforced preventive — `mdatron verify --deny-warnings` self-validate | bound — the canonical lexicon is registered (`E0090` over the methodology docs; retired spellings `reserved`, so `E0092` fires corpus-wide) and the letter-plus-number cluster prohibition (`E0091`) holds corpus-wide over an allow-list of reference-shaped clusters; §5's all-caps prohibition is mechanized by nothing yet and holds as conduct |
