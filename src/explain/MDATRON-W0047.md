...
pattern rule context. The schema family (JSON Schema over frontmatter)
...
a file expects the schema family (it declares a `schema_class`) and nothing can serve that
...
- A **Schematron-only** project — a `schema_class` selected by a pattern rule
  `context:` — is validated by the rule DSL, so the absent `schemas/` is not a
  false-clean and does not warn.
- A file with **no `schema_class`** requests no schema validation, so a missing
...
`context: <class>` to validate it via the rule DSL; or correct the `schema_class` on
