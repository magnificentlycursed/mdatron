...
that does not exist in the working tree. Links are found by a full CommonMark
...
`[readme](../README.md)` means the repository README. The working tree is
authoritative: uncommitted files count as present, and no git history is
consulted. Link checking is per-route opt-in (`links: true` in
